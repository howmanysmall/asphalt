...
pub struct SpriteInfo {
    ...
    /// Animation metadata (only present if this is an animation)
    #[serde(skip_serializing_if = "Option::is_none")]
    pub animation: Option<AnimationInfo>,
}

/// Information about an animation in the atlas
#[derive(Debug, Clone, Serialize, Deserialize)]
...
/// Layout information for animations
#[derive(Debug, Clone, Serialize, Deserialize)]
...
impl AtlasManifest {
    ...
    pub fn generate_luau(&self, asset_id: Option<u64>) -> String {
        ...
        for (name, sprite) in &self.sprites {
            ...
            // Add animation metadata if present
            if let Some(animation) = &sprite.animation {
                ...
                code.push_str(&format!(
                    "            frameSize = Vector2.new({}, {}),\n",
                    animation.frame_size.width, animation.frame_size.height
                ));

                // Format layout based on type
                let layout_str = match &animation.layout {
                    AnimationLayoutInfo::HorizontalStrip => "\"horizontal_strip\"".to_string(),
                    AnimationLayoutInfo::VerticalStrip => "\"vertical_strip\"".to_string(),
                    AnimationLayoutInfo::Grid { columns } => {
                        format!("{{ type = \"grid\", columns = {} }}", columns)
                    }
                };
    ...
    pub fn generate_typescript(&self) -> String {
        ...
        // Type definitions
        code.push_str("export type AnimationLayout =\n");
        code.push_str("    | \"horizontal_strip\"\n");
        code.push_str("    | \"vertical_strip\"\n");
        code.push_str("    | { type: \"grid\"; columns: number };\n\n");

        code.push_str("export interface Animation {\n");
        code.push_str("    readonly frameCount: number;\n");
        code.push_str("    readonly frameSize: Vector2;\n");
        code.push_str("    readonly layout: AnimationLayout;\n");
        code.push_str("    readonly frameDurationMs: number;\n");
        code.push_str("    readonly loops: boolean;\n");
        code.push_str("}\n\n");
        ...
        code.push_str("    readonly animation?: Animation;\n");
        code.push_str("}\n\n");
